    ...
    describe("GET /advertisements", () => {
      it("should return a status code 200 and all advertisements", (done) => {
        request.get(base, (err, res, body) => {
            ...
            expect(err).toBeNull();
            expect(body).toContain("Our Advertising Partners");
            expect(body).toContain("Mentos");
            done();
        })
      })
    })
    ...
    describe("POST /advertisements/create", () => {
        ...
        it("should create a new advertisement and redirect", (done) => {
          request.post(options,
            (err, res, body) => {
                Advertisement.findOne({where: {title: "Wendy's"}})
              .then((advertisement) => {
                expect(res.statusCode).toBe(303);
                expect(advertisement.title).toBe("Wendy's");
                expect(advertisement.description).toBe("Where's the beef?");
                done();
              })
              .catch((err) => {
                console.log(err);
